...
- [ ] Security headers (`SecurityHeadersMiddleware`), rate limiting (`slowapi`), limite de tamanho de body, validação de `aud`/`iss` do JWT (RS256, `JWT_PUBLIC_KEY_PATH` — ver ADR 0004) e o guard fail-fast de CORS sem origem configurada em produção permanecem ativos; nenhuma dessas proteções foi desabilitada ou contornada para viabilizar uma implementação ou teste.
